#!/usr/bin/env python3
"""
Generate balanced synthetic dataset with both normal and misconfigured devices.
...
# Generate synthetic data - balanced distribution
n_normal = 50
n_misconfig = 50
...
# Write to CSV
output_file = 'synthetic_balanced_test_data.csv'
fieldnames = [
...
print(f'✅ Created balanced synthetic dataset with {len(devices)} devices')
print(f'   Saved to: {output_file}')
print(f'\nDistribution:')
for label, count in sorted(label_counts.items()):
    print(f'   {label_names[label]} (label {label}): {count} devices ({count/len(devices)*100:.1f}%)')
print(f'\nTotal: {len(devices)} devices')
print(f'   Normal: {label_counts.get(0, 0)} devices')
print(f'   Misconfigured: {sum(v for k, v in label_counts.items() if k != 0)} devices')
print(f'\nFirst 10 rows:')
